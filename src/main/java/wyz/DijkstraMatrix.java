...
public class DijkstraMatrix {
    ...
    //打印出floyd各个路径
    public void print_floyd(int i,int j,int prev[][]){
        System.out.print(i+"  ");
        if(prev[i][j] !=j)
            printtt(prev[i][j],j,prev);
        System.out.print(j+"  ");
        System.out.println();
    }
    private void printtt(int i,int j,int prev[][]){
        System.out.print(i+"  ");
        if(prev[i][j] !=j) {
            printtt(prev[i][j], j, prev);
        }
...
//        pG.kruskal(); // Kruskal算法生成最小生成树
        ...
        //可根据prev来打印出最短路径
        pG.print_shortest(3,pG.mVexs.length,prev);
